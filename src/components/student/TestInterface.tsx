...
export const TestInterface: React.FC<TestInterfaceProps> = ({
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col font-sans w-full">
      ...
      <div className="sticky top-0 z-30 shadow-md bg-slate-900 border-b border-slate-800">
        {/* EXAM TOP HEADER BAR */}
        <header className="bg-slate-900 text-white px-3 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <h1 className="font-black text-xs sm:text-sm md:text-base text-white truncate leading-tight">{test.title}</h1>
              <p className="text-[10px] sm:text-[11px] text-slate-400 truncate mt-0.5">
                Candidate: <span className="text-blue-300 font-bold">{studentData.student_name}</span>
                {studentData.student_district && ` (${studentData.student_district})`}
              </p>
            </div>
          </div>
        ...
        {/* Section Switcher Bar - Mobile Safe with Full Visibility */}
        {sectionsList.length > 0 && (
          <div className="bg-slate-800/95 backdrop-blur-md text-slate-300 px-3 sm:px-6 py-2 border-t border-slate-800 flex items-center gap-2 overflow-x-auto text-xs font-bold scrollbar-none w-full">
            <span className="text-amber-400 uppercase text-[10px] tracking-wider shrink-0 font-black flex items-center gap-1">
              <span>Section:</span>
            </span>
            {sectionsList.map((sec) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl whitespace-nowrap transition-all flex items-center gap-1.5 cursor-pointer text-xs shrink-0 ${
                    isCurrentSection
                      ? 'bg-blue-600 text-white font-black shadow-xs ring-2 ring-blue-400'
                      : 'bg-slate-900 hover:bg-slate-700 text-slate-300 font-semibold border border-slate-700'
                  }`}
                >
                  <span>{sec}</span>
                </button>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 p-4 sm:p-7 md:p-8 shadow-xs space-y-5 sm:space-y-6 max-w-4xl mx-auto">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3 sm:pb-4">
                ...
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                  ...
                  {(currentQuestion.section || currentQuestion.subject) && (
                    <span className="px-2.5 py-1 bg-amber-500/15 text-amber-700 dark:text-amber-300 font-bold text-xs rounded-lg border border-amber-500/30 flex items-center gap-1">
                      <span className="text-[10px] uppercase font-black opacity-75">Section:</span>
