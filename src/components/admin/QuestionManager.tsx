...
export const QuestionManager: React.FC<QuestionManagerProps> = ({
  ...
  const handleDeleteSelected = async () => {
    ...
    if (window.confirm(`Kya aap selected ${count} questions ko is Mock Test se unlink/remove karna chahte hain?\n\n✓ Note: Ye questions aapke Question Bank me 100% safe rahenge aur wahan se delete nahi honge.`)) {
      ...
      setSelectedQuestionIds(new Set());
      notify('info', `${count} questions is Mock Test se unlink ho gaye. (Question Bank me safe hain)`);
    }
  };
  ...
  const confirmDeleteQuestion = async () => {
    ...
    notify('info', 'Question is Mock Test se unlink ho gaya. (Question Bank me safe hai)');
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                title="Unlink selected questions from this mock test (Safe in Question Bank)"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>Delete / Unlink Selected ({selectedQuestionIds.size})</span>
              </button>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {/* DU-XQE Mutate MCQ Button */}
                <button
                  onClick={() => setMutatingQuestion(q)}
                  className="px-2.5 py-1 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900/60 dark:hover:bg-purple-800/60 text-purple-900 dark:text-purple-200 font-bold text-xs rounded-lg border border-purple-300 dark:border-purple-700 flex items-center gap-1 cursor-pointer transition-all shadow-xs mr-1"
                  title="Mutate with DU-XQE: Generate a novel cognitive perspective variant for this mock test"
                >
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-lg border border-slate-200 dark:border-slate-700 flex items-center gap-1 cursor-pointer disabled:opacity-50 transition-all shadow-xs mr-1"
      ...
      {/* Delete/Unlink Question Confirmation Modal */}
      {deletingQuestionId && (
        <Modal
          isOpen={!!deletingQuestionId}
          onClose={() => setDeletingQuestionId(null)}
          title="Unlink Question from Mock Test"
        >
          <div className="space-y-4">
            <p className="text-sm text-slate-700 dark:text-slate-300">
              Kya aap is question ko is Mock Test se unlink/remove karna chahte hain?
            </p>
            <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 rounded-xl border border-emerald-200 dark:border-emerald-800 text-xs text-emerald-900 dark:text-emerald-200 font-medium">
              ✓ <strong>Safe in Question Bank:</strong> Ye question aapke Question Bank me 100% safe rahega aur wahan se delete nahi hoga.
            </div>
